# -*- coding: utf-8 -*-
'''
    Anime Metahandler - a plugin for Kodi
    Copyright (C) 2016 dat1guy

    This file is part of Anime Metahandler.

    Anime Metahandler is free software: you can redistribute it and/or modify
    it under the terms of the GNU General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    Anime Metahandler is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU General Public License for more details.

    You should have received a copy of the GNU General Public License
    along with Anime Metahandler.  If not, see <http://www.gnu.org/licenses/>.
'''
...
'''
import metahandler
from animetmdb import AnimeTMDB
...
from dat1guy.shared.helper import Helper


helper = Helper('script.module.animemetahandler', argv=sys.argv)
...
class AnimeMetaData(MetaData):
    ...
    def get_episode_meta_with_tvdb_id(self, tvshowtitle, tvdb_id, season, episode, air_date='', episode_title=''):
        helper.log('Attempting to retrieve episode meta data for: tvdbid: %s, season: %s, episode: %s, air_date: %s' % (tvdb_id, season, episode, air_date))

        if not tvdb_id:
            helper.log('Cannot find episode meta data without a tvdb_id; given %s' % tvdb_id)
            return {}
        if not season:
            season = 0
        if not episode:
        ...
        imdb_id = self._get_imdb_id(tvshowtitle, tvdb_id)
        ...
        if not meta:
            ...
            if not meta:
                meta = self._MetaData__init_episode_meta(imdb_id, tvdb_id, episode_title, season, episode, air_date)          
        ...
        helper.log('Returned Meta: %s' % meta)
        return meta
    ...
    def _get_imdb_id(self, name, tvdb_id):
        name = self._clean_string(name.lower())

        if tvdb_id:
            sql_select = "SELECT imdb_id FROM tvshow_meta WHERE tvdb_id = '%s'" % tvdb_id
        elif name:
            sql_select = "SELECT imdb_id FROM tvshow_meta WHERE title = '%s'" % name
        else:
            return None

        helper.log('Retrieving IMDB ID')
        helper.log('SQL SELECT: %s' % sql_select)
        ...
        try:
            ...
            helper.log_error('************* Error attempting to select from tvshow_meta table: %s ' % e)
            pass

        if matchedrow:
            match = dict(matchedrow)['imdb_id']
    ...
    def _cache_lookup_episodes(self, imdb_id, tvdb_id, first_air_date, season, num_episodes):
        ...
        '''
        helper.start('AnimeMetadata._cache_lookup_episodes')
        ...
        sql_select = ('SELECT '
                      ...
                      'episode_meta.tvdb_id as tvdb_id, '
        ...
        helper.end('AnimeMetadata._cache_lookup_episodes success')
        return meta_list
